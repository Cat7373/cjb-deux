...
public class mod_cjb_moreinfo extends BaseMod {
	...
	public void load()
	{
		...
		CJB.showemerald = CJB_Settings.getBoolean("moreinfo.showemerald", true);
	...
	public void renderMod(Minecraft mc)
	{
		...
		if (mc.currentScreen != null && !(mc.currentScreen instanceof CJB_GuiOptions))
			return;
        ...
        if (curritem != null) {
        	...
        	ItemName = curritem.itemID + ":" + curritem.getItemDamage() + " " + curritem.getTooltip(plr, false).get(0);
        ...
        if (CJB.showslimes) {
        	Chunk chunk = CJB.w.getChunkFromBlockCoords(MathHelper.floor_double( plr.posX), MathHelper.floor_double( plr.posZ));
        ...
        sLL = (!CJB.hideicons ? "" : "LightLevel: ") + CJB.w.getBlockLightValue(MathHelper.floor_double(plr.posX), MathHelper.floor_double(plr.posY), MathHelper.floor_double(plr.posZ));
        sBio = (!CJB.hideicons ? "" : "Biome: ") + CJB.w.getBiomeGenForCoords(MathHelper.floor_double(plr.posX), MathHelper.floor_double(plr.posZ)).biomeName;
        ...
        if (mc.isSingleplayer() && CJB.showrain)drawInfo(mc, sRain, x, posup ? y + (i++ * (i1 / j)) : y - (i++ * (i1 / j)), 240, 73);
        if (mc.isSingleplayer() && CJB.showthunder)drawInfo(mc, sThunder, x, posup ? y + (i++ * (i1 / j)) : y - (i++ * (i1 / j)), 240, 89);
        if (mc.isSingleplayer() && CJB.showslimes && slimes)drawInfo(mc, sSlimes, x, posup ? y + (i++ * (i1 / j)) : y - (i++ * (i1 / j)), 224, 89);
        if (mc.isSingleplayer() && CJB.showmobinfo && entity != null) drawInfo(mc, sMobInfo, x, posup ? y + (i++ * (i1 / j)) : y - (i++ * (i1 / j)), 0, 0);
	...
	private EntityLiving GetEntityLiving(World world, EntityPlayer plr)
	{
		...
		Iterator elist = list.iterator();
				
		while(elist.hasNext()) {
			Object obj = elist.next();
        	...
        	Vec3 vec3d = plr.getLook(1.0F).normalize();
            Vec3 vec3d1 = Vec3.vec3dPool.getVecFromPool(entity.posX - plr.posX, (entity.boundingBox.minY + (double)(entity.height / 2.0F)) - plr.posY + (double)plr.getEyeHeight(), entity.posZ - plr.posZ);
            double d = vec3d1.lengthVector();
            vec3d1 = vec3d1.normalize();
            double d1 = vec3d.dotProduct(vec3d1);
            if(d1 > 1.0D - 0.025D / d && plr.canEntityBeSeen(entity))
            {
                return entity;
            }
		}
