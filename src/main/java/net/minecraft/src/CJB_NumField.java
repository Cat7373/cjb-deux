...
public class CJB_NumField extends Gui
{
    /**
    ...
    private boolean isHexField = false;
    private int max = 0;
    private int min = 0;
    ...
    public CJB_NumField(FontRenderer par1FontRenderer, int par2, int par3, int par4, int par5)
    {
        text = "0";
        number = 0;
        maxStringLength = 32;
    ...
    private void processNumber() {
    	...
    	if (!text.isEmpty() && !(text.startsWith("-") && text.length() < 2)) {
	    	if (isHexField)
    ...
    public boolean textboxKeyTyped(char par1, int par2)
    {
        ...
        for (int i = 0 ; i < allowedchars.length() ; i++) {
        	if (allowedchars.charAt(i) == par1) {
        		func_50031_b(Character.toString(par1));
                return true;
        	}
        }
        
        if (!isHexField && textCharacterLength == 0 && par1 == '-') {
        	func_50031_b("-");
        }
    ...
    public boolean getIsHexField() {
    	return isHexField;
    }
    
    public void setIsHexField(boolean flag) {
    	isHexField = flag;
    }
    ...
    public String getNumberAsHexString() {
    	return Integer.toHexString(number);
    }
    
    private void changeNumberValue(int i) {
    	number += i;
