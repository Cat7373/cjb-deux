...
public class CJB_Minimap implements Runnable
{
        ...
        label2841:
        {
            try
            {
                if (mc != null)
                {
                    ...
                    if (this.thePlayer != mc.thePlayer) {
                    	this.thePlayer =  mc.thePlayer;
                    	playerdead = false;
                    }

                    if (this.theWorld != CJB.w)
                    {
                        ...
                        if (this.theWorld != null)
                        {
                            ...
                            this.nosky = theWorld.provider.hasNoSky;
                        }

                        this.stripCounter.reset();
                    }
                    
                    if (this.thePlayer != null && mc.currentScreen != null && mc.currentScreen instanceof GuiGameOver && !playerdead)
            		{
                    	playerdead = true;
                		...
                		loc.posy = this.thePlayer.posY + 1;
                    ...
                    GL11.glScaled(1.0D / this.scaledResolution.getScaleFactor(), 1.0D / this.scaledResolution.getScaleFactor(), 1.0D);
    ...
    private void renderRoundMap()
    {
        ...
        if (this.mapScale == 0)
        {
            var1 = this.scaledResolution.getScaleFactor() * (1.2f - 0.1f * CJB.mmsize);
        }
        ...
        var2 = this.scaledResolution.getScaleFactor() + 1 >> 1;
    ...
    private void renderSquareMap()
    {
        ...
        int var2 = this.scaledResolution.getScaleFactor() + 1 >> 1;
    ...
    private boolean checkGuiScreen()
    {
    	GuiScreen c = theMinecraft.currentScreen;
    	
    	if (c == null) return true;
    	if (c instanceof CJB_GuiMinimap) return true;
    ...
    private int getEntityColor(Entity ent)
    {
    	...
    	if ((!CJB.mmmobs || !CJB.pmentity) && ent instanceof EntityLiving && !(ent instanceof EntityPlayer))
    		return 0;
