...
public class CJB_ItemInWorldManager extends ItemInWorldManager
{
    ...
    public CJB_ItemInWorldManager(EntityPlayerMP player, World par1World)
    {
    	super(par1World);
    ...
    public void updateBlockRemoving()
    {
        ...
        {
        	if (!this.gameType.isAdventure() && !this.isCreative() && CJB.modcheats && CJB.onehitblock && System.currentTimeMillis() - hits > (CJB.breakspeed ? 0 : 200)) {            
                Block block = Block.blocksList[theWorld.getBlockId(partiallyDestroyedBlockX, partiallyDestroyedBlockY, partiallyDestroyedBlockZ)];
                if (block != null && block.getBlockHardness(theWorld, partiallyDestroyedBlockX, partiallyDestroyedBlockY, partiallyDestroyedBlockZ) > -1F) {
                    theWorld.playAuxSFX(2001, partiallyDestroyedBlockX, partiallyDestroyedBlockY, partiallyDestroyedBlockZ, block.blockID + (theWorld.getBlockMetadata(partiallyDestroyedBlockX, partiallyDestroyedBlockY, partiallyDestroyedBlockZ) << 12));
                	tryHarvestBlock(partiallyDestroyedBlockX, partiallyDestroyedBlockY, partiallyDestroyedBlockZ);
                }
        	}
    ...
    private boolean removeBlock(int par1, int par2, int par3)
    {
        ...
        if (var4 != null)
        {
            ...
            if (CJB.mobspawner && var4 == Block.mobSpawner) {
            	mod_cjb_cheats.dropBlockAsItem(theWorld, par1, par2, par3, new ItemStack(var4,1));
            }
        }
    ...
    {
        return CJB.modcheats && CJB.extendedreach && !thisPlayerMP.capabilities.isCreativeMode ? 200F : super.getBlockReachDistance();
    }
    @Override
